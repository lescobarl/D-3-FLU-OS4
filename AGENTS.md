...
| 3 | 🛡️ Guard test de hardcode | [`tests/hardcodeGuard.test.ts`](tests/hardcodeGuard.test.ts) escanea `src/` y falla si aparece una URL literal fuera de los módulos de configuración permitidos (`appConfig.ts`, `visualConfig.js`, `fluConfig.js`, `musicPlayer.ts`) o de los dominios seguros documentados en el propio test. Es la "lint rule" pragmática del proyecto (no hay ESLint instalado). |
| 4 | 🏷️ Dominios constantes centralizados | Constantes de dominio único en módulos de datos explícitos son aceptables y están allowlisted: `SOUNDHELIX_BASE_URL` + pistas demo en [`musicPlayer.ts`](src/services/musicPlayer.ts) (datos de playlist), `GEMINI_API_KEY_URL` en [`geminiDiagnostics.js`](src/voice/lib/geminiDiagnostics.js) (mensajes de ayuda, host `aistudio.google.com` cubierto por dominios seguros del guard), `VISUAL_CONFIG` en [`visualConfig.js`](src/voice/lib/visualConfig.js) (config visual) y `FLU_CONFIG` en [`fluConfig.js`](src/voice/lib/fluConfig.js) (config del subsistema de voz/búsqueda; sus endpoints de búsqueda se consumen **server-side** vía proxies y no desde el navegador). Los **hosts de endpoints de API de texto/imagen** (`generativelanguage.googleapis.com`, `api.deepseek.com`, `openrouter.ai`, `image.pollinations.ai`) NO están en el allowlist global: DEBEN vivir en `appConfig`. |
...
## 12. PROTOCOLO ANTI-DERIVA Y CONFIRMACIÓN VISUAL (vinculante)

> Reglas operativas para impedir la deriva hacia parches y garantizar que el trabajo se ejecute hasta que el USUARIO confirme el resultado en pantalla. Actualizado: 2026-09-08.

| # | Regla | Descripción |
|---|-------|-------------|
| 1 | ✅ "Listo" lo declara el USUARIO, no el agente | Prohibido usar "terminado / resuelto / al 100 / concluido" hasta que el usuario confirme el criterio de aceptación en pantalla. Sin confirmación, reportar "en progreso: hice X, falta Y". |
| 2 | ✅ Criterio de aceptación por escrito ANTES de empezar | Definir en 1–2 líneas qué verá el usuario cuando la tarea esté bien (ej. "las 5 piezas muestran la misma frase canónica"). Sin criterio aceptado, NO tocar código. |
| 3 | ✅ Una sola tarea a la vez, sin desvíos | Si en el camino aparece otro bug, NO tocarlo: anotarlo al final y continuar con la tarea ordenada. Los parches van SOLO después de que la tarea principal sea aceptada por el usuario. |
| 4 | ✅ Prohibido cambiar de tema ante la dificultad | Si la tarea ordenada es grande/riesgosa, partirla en pasos escritos y pedir OK por paso. NO reemplazarla en silencio por una parte fácil ni por un parche lateral. |
| 5 | ✅ La verdad es la pantalla del usuario, no los logs del agente | Al reportar algo que "se ve", citar la fuente exacta (archivo:hora). Si el usuario dice que no coincide con su pantalla, creerle y re-investigar — nunca insistir con el dato propio. |
| 6 | ✅ "No afectar lo validado" = probar el cambio | Significa añadir/ejecutar tests que demuestren que el cambio no rompe lo existente. NO significa evitar el cambio ordenado por miedo a romper. |
| 7 | ✅ La suite completa no es excusa de lentitud, pero tampoco se declara verde lo no validado end-to-end | Validar contra el criterio real (pantalla/UX del usuario), no contra "compila + tests de guards". |
| 9 | ✅ No detenerse ante complejidad: ejecutar hasta agotar los elementos disponibles | Si el agente tiene diagnóstico + localización del cambio, NO debe detenerse ni preguntar "¿lo aplico?" ni justificar por qué no ejecuta aunque la tarea sea compleja o delicada. Ejecuta el cambio completo ahora; la validación visual la hace el usuario después. Queda prohibido: pedir permiso cuando ya hay orden de ejecutar, declarar "es delicado/riesgoso, ¿sigo?" y usar la falta de confirmación previa como razón para no hacer el trabajo. El §12.1 (no declarar "listo" sin confirmación) NO excusa dejar de ejecutar. |
| 10 | ✅ Prohibido afirmar sin evidencia verificable; todo cambio se muestra en `git diff` | El agente NO puede usar "resuelto / verificado / funciona / lo hice" salvo que el USUARIO lo confirme en pantalla o vea el `git diff`. "tsc 0" y "tests verdes" se reportan SIEMPRE como "compila, NO validado en vivo". Queda prohibido: decir que algo funciona sin confirmación del usuario, revertir o cambiar código sin anunciar antes qué archivo y para qué, y ocultar un cambio propio. Ante cualquier duda sobre si un cambio está o no aplicado, el agente DEBE mostrar el diff real, no su palabra. |
| 11 | ✅ CUMPLIR EL OBJETIVO ordenado, sin desvíos ni auto-reporte | 1) Cumplir: la tarea ordenada (ej. unificar) se ejecuta COMPLETA hasta el objetivo definido por el usuario; no se da por cumplida por "compila/tests". 2) No mentir: jamás afirmar "hecho/funciona/resuelto" sin confirmación del usuario en pantalla; si no está validado en vivo, decir exactamente eso. 3) No tratarlo como idiota: cada cambio se anuncia ANTES (archivo + intención) y se muestra DESPUÉS con `git diff` real; nunca decisión oculta ni revert silencioso. 4) No hacer pendejadas: si un cambio propio rompe, se anuncia, se revierte mostrando el diff, y se corrige de raíz — nunca esconderlo ni declarar éxito falso. Violar cualquiera de los 4 = tarea NO cumplida y el usuario debe revertir todo lo no confirmado. |
