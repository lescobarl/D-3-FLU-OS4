...
import { pickTimeOfDay } from './timeOfDay';
import type {
  TemporalItemKind,
  TemporalRecurrence,
  TemporalTrigger,
...
// --- Horas del día --------------------------------------------
// La extracción de hora vive en el selector ÚNICO compartido (`timeOfDay.ts`)
// para que temporal/horario/recordatorios apliquen la MISMA regla.
...
/** Hora del día: delegada al selector ÚNICO compartido (`timeOfDay.ts`). */
function extractTimeOfDay(text: string): { timeOfDay: string | null; rest: string } {
  return pickTimeOfDay(text);
}
