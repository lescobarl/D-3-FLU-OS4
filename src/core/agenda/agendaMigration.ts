...
// Migración PURA (testeable) de las 3 tablas viejas → `agenda`.
// ------------------------------------------------------------
...
//   - HorarioRecord   → 'clase' weekly (día + hora de inicio).
// - `dia` de horario es 1-7 (1=Lunes); agenda usa 0-6 (0=Domingo).
...
function horarioToAgenda(h: HorarioRecord): AgendaItem {
    // dia 1-7 (1=Lunes) → 0-6 (0=Domingo).
    const day = h.dia % 7;
    return {
        id: h.id,
        kind: 'clase',
...
/** Copia las 3 fuentes viejas a items de agenda (sin dedup ni borrado). */
export function mapLegacyToAgenda(rows: LegacyRows): AgendaItem[] {
    ...
    for (const h of rows.horario ?? []) out.push(horarioToAgenda(h));
    return out;
}
